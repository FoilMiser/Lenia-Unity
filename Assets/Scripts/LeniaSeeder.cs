...
public class LeniaSeeder : MonoBehaviour {
  ...
  void Start(){ if(!sim) sim = FindObjectOfType<LeniaSimulator>(); if(autoSeedOnPlay){ SeedNoise(); if(addOrbium) SeedOrbium(); } }
  ...
  public void SeedOrbium(){
    ...
    for(int y=0;y<h;y++) for(int x=0;x<w;x++){
      ...
      float ring=Mathf.Exp(-0.5f*Mathf.Pow((r-R)/(0.22f*R),2f)); // Gaussian ring
